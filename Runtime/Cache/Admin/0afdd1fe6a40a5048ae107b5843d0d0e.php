  ...
  <div class="col-sm-12">
      ...
      <table data-toggle="table" data-click-to-select="true" data-mobile-responsive="true">
          ...
          <?php if(is_array($codata)): $i = 0; $__LIST__ = $codata;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$v): $mod = ($i % 2 );++$i;?><tr>
              ...
              <td>
                  <button onclick="_openLayerUrl('<?php echo U('addclasses', array('cid'=>$v['cid']));?>')" class="btn btn-info " type="button"><i class="fa fa-plus"></i> 添加专业</button>
...
function _del(obj){
  ...
  _ajaxmodify({url:'/admin.php/Classes/del',msg:'您确定要删除这个院系？',check:"确定",data:{id:oid}});
